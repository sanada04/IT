...
--- 貸し出し車両を満タン近くに（燃料スクリプトが無い・失敗時はネイティブ）
local function setTaxiSpawnFuel(veh)
    local level = tonumber(Config.SpawnFuelLevel) or 100.0
    if level > 100.0 then level = 100.0 end
    if level < 0.0 then level = 0.0 end
    local ok = false
    local res = Config.FuelResource
    if res and type(res) == 'string' and res ~= '' then
        ok = pcall(function()
            exports[res]:SetFuel(veh, level)
        end)
    end
    if not ok then
...
--- qb-vehiclekeys（vehiclekeys:client:SetOwner）。ネットワーク・プレート確定後に再送
local function giveTaxiVehicleKeys(veh)
