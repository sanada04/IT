...
function updateBgmToggleButton() {
    const isPaused = !background || background.paused;
    $('#bgm-toggle').text(isPaused ? 'BGM: OFF' : 'BGM: ON');
}

$(document).ready(function (){
    updateBgmToggleButton();

    $(document).on('click', '#bgm-toggle', function () {
    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            ...
            if (data.toggle) {
                ...
                // 待ち＋START 画面を挟まず、キャラ取得後すぐスロット一覧へ（背後の 3D が見えにくくする）
                setTimeout(function () {
                    setCharactersList();
...
function setCharactersList() {
    ...
    htmlResult += '<div class="character-btn" id="play"><p id="play-text">Select a character</p></div>'
