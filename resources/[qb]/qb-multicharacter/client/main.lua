...
local function destroyCharSelectCam()
...
local function setupCharSelectCamera()
    TriggerEvent('qb-weathersync:client:DisableSync')
    SetTimecycleModifier('hud_def_blur')
    SetTimecycleModifierStrength(1.0)
    FreezeEntityPosition(PlayerPedId(), false)
    destroyCharSelectCam()
...
local function openCharMenu(bool)
    QBCore.Functions.TriggerCallback("qb-multicharacter:server:GetNumberOfCharacters", function(result)
        ...
        -- 先にスクリプトカメラを有効化してからロード画面を閉じる（解除直後のデフォルト視点／自キャラの一瞬表示を防ぐ）
        if bool and not loadScreenCheckState then
            setupCharSelectCamera()
            ShutdownLoadingScreen()
            ShutdownLoadingScreenNui()
            loadScreenCheckState = true
            -- ShutdownLoadingScreenNui が NUI フォーカスを落とすため、マルチキャラ UI 用に付け直す
            Wait(0)
