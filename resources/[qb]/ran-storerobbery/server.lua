...
lib.callback.register("ran-houserobbery:server:getPrize", function(source, prize, storeid, regid)
    ...
    if Config.RegisterOncePerStore and config.registerLooted then return end
    ...
    if Config.RegisterOncePerStore then
        config.registerLooted = true
        for _, r in pairs(config.registar) do
            r.robbed = true
        end
    end
