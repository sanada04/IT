...
RegisterNetEvent('qb-spawn:client:setupSpawns', function(cData, new, apps)
    ...
    elseif new then
        -- 初回キャラ作成: アパートマップではなく市役所スポーンのみ（NUI 側でボタンを絞る）
        SendNUIMessage({
            action = "setupFirstCharacterSpawn",
            spawns = QB.Spawns,
        })
    end
end)
...
RegisterNUICallback('spawnplayer', function(data, cb)
    ...
    elseif type == "normal" then
        ...
        local firstChar = data.firstCharacterSpawn == true
        if firstChar then
            -- 新規キャラ作成直後はマルチキャラがランダムバケットに入れるため、先にメインワールドへ戻す
        ...
        PostSpawnPlayer()
        if firstChar then
            local aptType = QB.FirstSpawnApartmentType
            if aptType and Apartments and Apartments.Locations and Apartments.Locations[aptType] then
                TriggerServerEvent('apartments:server:CreateApartment', aptType, Apartments.Locations[aptType].label)
            end
