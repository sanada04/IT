window.addEventListener('message', function (event) {
	...
	if (item.showmenu){
		...
		$('#new-contracts-1').append((config.cooldown*2) + '分ごとに新しい契約が追加されます');
		$('#new-contracts-2').empty();
		$('#new-contracts-2').append((config.cooldown*2) + '分ごとに新しい契約が追加されます');
		...
		for (const contract of contracts) {
					...
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div><i class="fas fa-route"></i><span class="ml-2">距離: ` + contract.distance + `km</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i><span class="ml-2">報酬: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(contract.reward) + `</span></div>
							</div>
						</div>
					</div>
			...
			if(contract.cargo_type == 1) {
				list_item += '<div data-tooltip="爆発物"><img class="rounded-circle" src="img/explosive-1.png" width="30"></div>';
			}else if(contract.cargo_type == 2) {
				list_item += '<div data-tooltip="可燃性ガス"><img class="rounded-circle" src="img/flamable-2.png" width="30"></div>';
			}else if(contract.cargo_type == 3) {
				list_item += '<div data-tooltip="可燃性液体"><img class="rounded-circle" src="img/flamable-3.png" width="30"></div>';
			}else if(contract.cargo_type == 4) {
				list_item += '<div data-tooltip="可燃性固体"><img class="rounded-circle" src="img/flamable-4.png" width="30"></div>';
			}else if(contract.cargo_type == 5) {
				list_item += '<div data-tooltip="毒性物質"><img class="rounded-circle" src="img/toxic-6.png" width="30"></div>';
			}else if(contract.cargo_type == 6) {
				list_item += '<div data-tooltip="腐食性物質"><img class="rounded-circle" src="img/corrosive-8.png" width="30"></div>';
			}
			if(contract.fragile == 1) {
				list_item += '<div data-tooltip="壊れやすい貨物"><img class="rounded-circle" src="img/fragile.png" width="30"></div>';
			}
			if(contract.valuable == 1) {
				list_item += '<div data-tooltip="高価値貨物"><img class="rounded-circle" src="img/valuable.png" width="30"></div>';
			}
			if(contract.fast == 1) {
				list_item += '<div data-tooltip="緊急貨物"><img class="rounded-circle" src="img/fast.png" width="30"></div>';
			}
			list_item += `
							</div>
						</div>
						<button onclick="startJob(` + contract.contract_id + `,` + contract.reward + `,` + contract.distance + `)" class="btn btn-blue btn-darken-2 white">仕事を開始</button>
					</div>
				</li>
			</ul>
			`;
		...
		$('#skills-desc').empty();
		$('#skills-desc').append("スキルを上げてより良い仕事を受けましょう（使用可能スキルポイント: " + users.skill_points + "）");
		...
		list_item = `
			<div class="col-12 mt-3 mb-1">
				<h4 class="text-uppercase">販売店</h4>
				<p>自分やドライバーのためにトラックを購入できます</p>
			</div>`;
			...
			list_item += `
				<div class="card"> <img src="` + truck.img + `" class="card-img-top" width="100%">
					<div class="card-body pt-0 px-0">
						<div class="d-flex flex-row justify-content-between mb-0 mt-3 px-3"> <span class="text-muted">トラック</span>
							<h6>` + truck.name + `</h6>
						</div>
						<hr class="mt-2 mx-3">
						<div class="d-flex flex-row justify-content-between px-3 pb-4">
							<div class="d-flex flex-column"><span class="text-muted">価格</span></div>
						...
						<div class="d-flex flex-row justify-content-between p-3 mid">
							<div class="d-flex flex-column"><small class="text-muted mb-1">エンジン</small>
							...
							<div class="d-flex flex-column"><small class="text-muted mb-2">馬力</small>
								<div class="d-flex flex-row"><img src="img/power_icon.jpg">
									<h6 class="ml-1">` + truck.hp + ` hp</h6>
								</div>
							</div>
						</div>
						<div class="mx-3 mt-3 mb-2"><button onclick="buyTruck('` + key + `','` + truck.price + `')" type="button" class="btn btn-blue btn-darken-2 white btn-block"><small>購入</small></button></div> <small class="d-flex justify-content-center text-muted">*注意事項</small>
					</div>
				</div>
				`;
		}
		...
		for (const truck of myTrucks) {
			...
			if(truck.driver == 0){
				...
				$('#diagnostic-body').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.body + ` %</h3><span>ボディ修理 (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.body)*config.valor_reparo.body) + `)</span>
						</div>
				...
				$('#diagnostic-engine').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.engine + ` %</h3><span>エンジン修理 (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.engine)*config.valor_reparo.engine) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-transmission').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.transmission + ` %</h3><span>トランスミッション修理 (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.transmission)*config.valor_reparo.transmission) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-wheels').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.wheels + ` %</h3><span>ホイール修理 (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.wheels)*config.valor_reparo.wheels) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><img src="` + config.concessionaria[truck.truck_name].img + `" class="img-radius img-width" alt="User-Profile-Image">
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div>
									<i class="fas fa-tools"></i><span class="ml-2">ボディ: ` + truck.body + `%</span>
								</div>
								<div class="ml-3">
									<i class="fas fa-tools"></i><span class="ml-2">エンジン: ` + truck.engine + `%</span>
								</div>
								<div class="ml-3">
									<i class="fas fa-tools"></i><span class="ml-2">トランスミッション: ` + truck.transmission + `%</span>
								</div>
								<div class="ml-3">
									<i class="fas fa-tools"></i><span class="ml-2">ホイール: ` + truck.wheels + `%</span>
								</div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						...
						<button onclick="sellTruck('` + truck.truck_id + `','` + truck.truck_name + `')" class="btn btn-red btn-accent-4 white">トラック売却</button>
					</div>
				</li>
				`;
		}
		...
		list_item = `
			<div class="col-12 mt-3 mb-1">
				<h4 class="text-uppercase">人材紹介所</h4>
				<p>会社で働く新しいドライバーを募集できます</p>
			</div>
			`;
		for (const driver of drivers) {
			if (driver.user_id == null || driver.user_id == undefined){
				list_item += `
					<div class="card user-card">
						<div class="card-block">
							...
							<p class="text-muted">料金: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price_per_km) + `/km</p>
							<hr>
							<p class="text-muted">製品種類</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.product_type) + `
							</ul>
							<p class="text-muted">距離</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.distance) + `
							</ul>
							<p class="text-muted">高価値貨物</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.valuable) + `
							</ul>
							<p class="text-muted">壊れやすい貨物</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.fragile) + `
							</ul>
							<p class="text-muted">時間厳守配送</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.fast) + `
							</ul>
							<div onclick="hireDriver('` + driver.driver_id + `')" class="mx-3 mt-3 mb-2"><button type="button" class="btn btn-blue btn-darken-2 white btn-block"><small>雇用</small></button></div>
						</div>
					</div>
					`;
			}else{
					...
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center">
							...
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 date-time">
									<div>
										<i class="fas fa-coins"></i><span class="ml-2">料金: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price_per_km) + `/km</span>
									</div>
									<div class="ml-3">
										<i class="fas fa-medal"></i><span class="ml-2">ドライバースキル: 製品種類 (` + driver.product_type + `) 距離 (` + driver.distance + `) 高価値貨物 (` + driver.valuable + `) 壊れやすい貨物 (` + driver.fragile + `) 時間厳守配送 (` + driver.fast + `)</span>
									</div>
								</div>
							</div>
						</div>
						<div class="d-flex flex-row align-items-center">
							...
							<button onclick="fireDriver('` + driver.driver_id + `')" class="btn btn-red btn-accent-4 white">解雇</button>
						</div>
					</li>
				`);
			}
		...
		$('#loan-title').empty();
		$('#loan-title').append('会社へ投資するためのローンを組めます（最大有効ローン: ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.max_emprestimo) + '）');
		
		$('#loan-1').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[0][0]) + '</h3><span>1日あたり ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[0][1]) + ' を返済</span>';
		$('#loan-1').append(list_item);
		$('#loan-2').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[1][0]) + '</h3><span>1日あたり ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[1][1]) + ' を返済</span>';
		$('#loan-2').append(list_item);
		$('#loan-3').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[2][0]) + '</h3><span>1日あたり ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[2][1]) + ' を返済</span>';
		$('#loan-3').append(list_item);
		$('#loan-4').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[3][0]) + '</h3><span>1日あたり ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[3][1]) + ' を返済</span>';
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div><i class="fas fa-coins"></i></i><span class="ml-2">残額: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(loan.remaining_amount) + `</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i></i><span class="ml-2">日次返済額: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(loan.day_cost) + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						<button onclick="payLoan('` + loan.id + `')" class="btn btn-red btn-accent-4 white">ローン返済</button>
					</div>
				</li>
			`;
...
function getDriverAvailableTrucksHTML(myTrucks,driver,config){
	...
	if (has_truck == null) {
		html = '<option selected="selected">トラックを選択</option>' + html;
	}else{
		html = '<option driver_id="' + driver.driver_id + '">トラックを選択</option>' + html;
	}
	return html;
}

function getMyTruckHTML(truck){
	return truck.driver==0 ? `<button onclick="spawnTruck(` + truck.truck_id + `)" class="btn btn-blue btn-darken-2 white white mr-2">トラック出庫</button> <button onclick="setDriver(null,'` + truck.truck_id + `')" class="btn btn-blue btn-darken-2 white white mr-2">解除</button>` : `<button onclick="setDriver('0','` + truck.truck_id + `')" class="btn btn-blue btn-darken-2 white white mr-2">トラック選択</button>`
}
