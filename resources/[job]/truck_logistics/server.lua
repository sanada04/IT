...
local vrp_ready = nil
local function isAllowedWebhook(url)
...
function SendWebhookMessage(webhook, message)
	if isAllowedWebhook(webhook) then
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local sql =
		"INSERT INTO `trucker_available_contracts` (contract_type, contract_name, coords_index, price_per_km, cargo_type, fragile, valuable, fast, truck, trailer) VALUES (@contract_type, @contract_name, @coords_index, @price_per_km, @cargo_type, @fragile, @valuable, @fast, @truck, @trailer);";
		MySQL.Sync.execute(sql,
			{ ['@contract_type'] = contract_type, ['@contract_name'] = contract_name, ['@coords_index'] = coords_index,
				['@price_per_km'] = (price_per_km * bonus), ['@cargo_type'] = cargo_type, ['@fragile'] = fragile,
				['@valuable'] = valuable, ['@fast'] = fast, ['@truck'] = truck, ['@trailer'] = trailer });
...
Citizen.CreateThread(function()
	...
	while true do
		...
		price = price +
		(product_type + distance + fragile + valuable + fast) *
		(price * (Config.motoristas.porcentagem_bonus_habilidades / 100))
		local price_per_km = math.random(Config.motoristas.preco_por_km_min, Config.motoristas.preco_por_km_max)
		price_per_km = price_per_km +
		(product_type + distance + fragile + valuable + fast) *
		(price_per_km * (Config.motoristas.porcentagem_bonus_habilidades / 100))
		...
		local sql =
		"INSERT INTO `trucker_drivers` (user_id, name, product_type, distance, fragile, valuable, fast, price, price_per_km, img) VALUES (NULL, @name, @product_type, @distance, @fragile, @valuable, @fast, @price, @price_per_km, @img);";
		MySQL.Sync.execute(sql,
			{ ['@name'] = name, ['@product_type'] = product_type, ['@distance'] = distance, ['@fragile'] = fragile,
				['@valuable'] = valuable, ['@fast'] = fast, ['@price'] = price, ['@price_per_km'] = price_per_km,
				['@img'] = driver.img });
...
Citizen.CreateThread(function()
	...
	while true do
		local sql =
		[[SELECT d.driver_id, d.user_id, d.name, d.product_type, d.distance, d.valuable, d.fragile, d.fast, d.price, d.price_per_km
		...
		for k, v in pairs(data) do
			...
			if Config.trabalhos.gera_dinheiro_offline or source then
				if tryGetTruckerMoney(v.user_id, v.price + v.price_per_km) then
					...
					amount = amount +
					(v.product_type + v.distance + v.fragile + v.valuable + v.fast) *
					(amount * (Config.trabalhos.porcentagem_bonus_habilidades / 100))
					...
					MySQL.Sync.execute(sql, { ['@driver_id'] = v.driver_id });
					if source then
						TriggerClientEvent("QBCore:Notify", source, Lang[Config.lang]['driver_failed']:format(v.name),
							"error")
					end
				end
		...
		for k, v in pairs(data) do
			if v.timer + Config.emprestimos.cooldown < os.time() then
				...
				if tryGetTruckerMoney(v.user_id, v.day_cost) then
					...
					if new_loan > 0 then
						local sql =
						"UPDATE `trucker_loans` SET remaining_amount = @remaining_amount, timer = @timer WHERE id = @id";
		...
		if user_id then
			...
			if query and query[1] then
				...
				if query_users and query_users[1] then
					local userProductType = toNumberOr(query_users[1].product_type, 0)
					local userFragile = toNumberOr(query_users[1].fragile, 0)
					local userValuable = toNumberOr(query_users[1].valuable, 0)
					local userFast = toNumberOr(query_users[1].fast, 0)
					local userDistanceLevel = toNumberOr(query_users[1].distance, 0)
					local contractCargoType = toNumberOr(query[1].cargo_type, 0)
					local contractFragile = toNumberOr(query[1].fragile, 0)
					local contractValuable = toNumberOr(query[1].valuable, 0)
					local contractFast = toNumberOr(query[1].fast, 0)
					local contractType = toNumberOr(query[1].contract_type, 0)
					local requiredDistance = toNumberOr(distance, 0)
					local maxDistance = toNumberOr(Config.habilidade_distancia[userDistanceLevel], 0)

					if userProductType >= contractCargoType then
						if userFragile >= contractFragile then
							if userValuable >= contractValuable then
								if userFast >= contractFast then
									if maxDistance >= requiredDistance then
										if contractType == 0 then
											...
											if query_truck and query_truck[1] then
												...
												TriggerClientEvent("Notify", source, "negado",
													Lang[Config.lang]['own_truck'])
											end
										end
									else
										TriggerClientEvent("QBCore:Notify", source, Lang[Config.lang]['no_skill_1'],
											"error")
									end
								else
...
AddEventHandler("truck_logistics:upgradeSkill", function(data)
	...
	if user_id then
		...
		if query.skill_points >= (data.value - query[data.id]) then
			local sql = "UPDATE `trucker_users` SET " ..
			data.id .. " = @value, skill_points = @skill_points WHERE user_id = @user_id";
			MySQL.Sync.execute(sql,
				{ ['@user_id'] = user_id, ['@value'] = data.value,
					['@skill_points'] = (query.skill_points - (data.value - query[data.id])) });
		...
		if query then
			...
			if amount > 0 then
				if tryGetTruckerMoney(user_id, amount) then
					local sql = "UPDATE `trucker_trucks` SET " .. item ..
					" = 1000 WHERE user_id = @user_id AND driver = 0";
...
AddEventHandler("truck_logistics:buyTruck", function(data)
	...
	if user_id then
		if tryGetTruckerMoney(user_id, tonumber(data.price)) then
			...
			SendWebhookMessage(Config.webhook,
				Lang[Config.lang]['logs_buytruck']:format(data.truck_name, data.price,
					user_id ..
					os.date("\n[" ..
	...
	if debug_cooldown[source] == nil then
		...
		if user_id then
			...
			if query then
				...
				local amount = math.floor(tonumber(Config.concessionaria[data.truck_name].price *
				Config.multiplicador_venda))
				giveTruckerMoney(user_id, amount)
				TriggerClientEvent("QBCore:Notify", source, Lang[Config.lang]['sold'], "success")
				SendWebhookMessage(Config.webhook,
					Lang[Config.lang]['logs_selltruck']:format(data.truck_name, amount,
						user_id ..
						os.date("\n[" ..
						Lang[Config.lang]['logs_date'] .. "]: %d/%m/%Y [" ..
						Lang[Config.lang]['logs_hour'] .. "]: %H:%M:%S")))
				openUI(source, true)
			end
		end
		debug_cooldown[source] = nil
	...
	if debug_cooldown[source] == nil then
		...
		if user_id then
			...
			if amount_loans + Config.emprestimos.valores[data.loan_id][1] <= getMaxEmprestimo(user_id) then
				local sql =
				"INSERT INTO `trucker_loans` (user_id,loan,remaining_amount,day_cost,taxes_on_day) VALUES (@user_id,@loan,@remaining_amount,@day_cost,@taxes_on_day);";
				MySQL.Sync.execute(sql,
					{ ['@user_id'] = user_id, ['@loan'] = Config.emprestimos.valores[data.loan_id][1],
						['@remaining_amount'] = Config.emprestimos.valores[data.loan_id][1],
						['@day_cost'] = Config.emprestimos.valores[data.loan_id][2],
						['@taxes_on_day'] = Config.emprestimos.valores[data.loan_id][3] });
...
AddEventHandler("truck_logistics:finishJob",
	function(data, distance, reward, truck_data, truck_engine, truck_body, trailer_body)
		local function toNumberOr(value, fallback)
			local n = tonumber(value)
			if n == nil then return fallback end
			return n
		...
		local user_id = xPlayer.PlayerData.citizenid
		if user_id then
			trailer_body = toNumberOr(trailer_body, 1000) / 1000
			local safeDistance = toNumberOr(distance, 0)
			local safeReward = toNumberOr(reward, 0)
			local exp_amount = safeReward * (Config.exp / 100)
			local bonus = 0
			local bonus_exp = 0
			...
			local queryFragile = toNumberOr(query.fragile, 0)
			local queryValuable = toNumberOr(query.valuable, 0)
			local queryFast = toNumberOr(query.fast, 0)
			local queryDistance = toNumberOr(query.distance, 0)

			if isEnabled(data and data.fragile) then
				local fragileMoneyBonus = toNumberOr(Config.bonus['fragile']['dinheiro'][queryFragile], 0)
				local fragileExpBonus = toNumberOr(Config.bonus['fragile']['exp'][queryFragile], 0)
				bonus = bonus + safeReward * (fragileMoneyBonus / 100)
				bonus_exp = bonus_exp + exp_amount * (fragileExpBonus / 100)
			end
			if isEnabled(data and data.valuable) then
				local valuableMoneyBonus = toNumberOr(Config.bonus['valuable']['dinheiro'][queryValuable], 0)
				local valuableExpBonus = toNumberOr(Config.bonus['valuable']['exp'][queryValuable], 0)
				bonus = bonus + safeReward * (valuableMoneyBonus / 100)
				bonus_exp = bonus_exp + exp_amount * (valuableExpBonus / 100)
			end
			if isEnabled(data and data.fast) then
				local fastMoneyBonus = toNumberOr(Config.bonus['fast']['dinheiro'][queryFast], 0)
				local fastExpBonus = toNumberOr(Config.bonus['fast']['exp'][queryFast], 0)
				bonus = bonus + safeReward * (fastMoneyBonus / 100)
				bonus_exp = bonus_exp + exp_amount * (fastExpBonus / 100)
			end
			if safeDistance > toNumberOr(Config.habilidade_distancia[0], 0) then
				if Config.bonus['distance']['dinheiro'][queryDistance] then
					local distanceMoneyBonus = toNumberOr(Config.bonus['distance']['dinheiro'][queryDistance], 0)
					local distanceExpBonus = toNumberOr(Config.bonus['distance']['exp'][queryDistance], 0)
					bonus = bonus + safeReward * (distanceMoneyBonus / 100)
					bonus_exp = bonus_exp + exp_amount * (distanceExpBonus / 100)
				end
			end
			local received_amount = math.floor((safeReward + bonus) * trailer_body)
			...
			if truck_data.truck_id then
				...
				MySQL.Sync.execute(sql, {['@engine'] = truck_engine, ['@body'] = truck_body, ['@transmission'] = math.floor((truck_engine + truck_body)/2), ['@wheels'] = tonumber(string.format("%.2f", safeDistance))*10, ['@truck_id'] = truck_data.truck_id});
			end
			...
			MySQL.Sync.execute(sql, {['@reward'] = received_amount, ['@distance'] = tonumber(string.format("%.2f", safeDistance)), ['@exp_amount'] = received_xp, ['@user_id'] = user_id});
	
			giveTruckerMoney(user_id,received_amount)
			TriggerClientEvent("QBCore:Notify", source,
				Lang[Config.lang]['reward']:format(tostring(received_amount), tostring(trailer_body * 100),
					tostring(received_xp)), "success")
