...
local function canAccessTrucker()
    if not Config.jobRequired then
        return true
    end

    local job = getCurrentJob()
    return job ~= nil and job.name == Config.jobName
end
...
local function createJobBlips()
    clearJobBlips()
    if not canAccessTrucker() then return end

    for _, v in pairs(Config.blips or {}) do
        jobBlips[#jobBlips + 1] = addBlip(v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8])
    end
end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for k,mark in pairs(Config.empresas) do
			...
			if not menuactive and distance <= 15.0 and canAccessTrucker() then
				timer = 1
...
AddEventHandler('truck_logistics:startContract', function(data,job_distance,reward,truck_data)
	...
	if truck and not data.contract_type ~= 0 then
	...
	loading = true
	local x,y,z,h
	if not data.contract_type ~= 0 then
	...
	while IsEntityAVehicle(truck) and not data.contract_type ~= 0 do 
		timer = 2000
		local ped = PlayerPedId()
		veh = GetVehiclePedIsIn(ped,false)
		for k,v in pairs(Config.empresas) do
