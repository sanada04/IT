...
lib.callback.register('tj_restaurants:giveItem', function(source, data)
    ...
    local itemAmount = tonumber(data.amount) or 1
    if itemAmount < 1 then itemAmount = 1 end

    if data.recipe then
        for k, v in pairs(data.recipe) do
            local requiredAmount = (tonumber(v.amount) or 1) * itemAmount
            if Config.Inventory == 'ox' then
                exports.ox_inventory:RemoveItem(source, v.name, requiredAmount)
            elseif Config.Inventory == 'qb' then
                xPlayer.Functions.RemoveItem(v.name, requiredAmount) 
            elseif Config.Inventory == 'qs' then
                exports['qs-inventory']:RemoveItem(source, v.name, requiredAmount)
            end
        end
    end
    ...
    if data.item then
        if Config.Framework == 'esx' and Config.Inventory ~= 'qs' then
            xPlayer.addInventoryItem(data.item, itemAmount)
        elseif Config.Framework == 'qb' and Config.Inventory ~= 'qs' then
            xPlayer.Functions.AddItem(data.item, itemAmount)
        elseif Config.Framework == 'esx' or Config.Framework == 'qb' and Config.Inventory ~= 'qs' then
            exports['qs-inventory']:AddItem(source, data.item, itemAmount)
        end
    end
end)
...
RegisterNetEvent('tj_burgershot:openShop', function(shopName)
    if Config.Inventory == 'ox' then
        TriggerClientEvent('ox_inventory:openInventory', source, 'shop', { type = shopName })
    elseif Config.Inventory == 'qb' then
...
RegisterServerEvent('tj_burgershot:openStash', function(stashName)
    if Config.Inventory == 'ox' then
        TriggerClientEvent('ox_inventory:openInventory', source, 'stash', stashName)
    elseif Config.Inventory == 'qb' then
