...
local waFramework = (GetResourceState('es_extended') == 'started') and 'esx' or 'qb'
...
startDeathAnimation = function()
    ...
    loadAnimDict('veh@bus@passenger@common@idle_duck', 15000)
		...
		ESX.TriggerServerCallback('wasabi_ambulance:itemCheck', function(quanity)
			if quanity > 0 then
				...
				if Player(targetId).state.dead then
					isBusy = true
					local ped = cache.ped
					loadAnimDict('mini@cpr@char_a@cpr_str', 15000)
		...
		ESX.TriggerServerCallback('wasabi_ambulance:itemCheck', function(quantity)
			if quantity > 0 then
				...
				if not Player(targetId).state.dead then
					...
					loadAnimDict('anim@amb@clubhouse@tutorial@bkr_tut_ig3@', 15000)
					local targetPed = GetPlayerPed(player)
					local tCoords = GetEntityCoords(targetPed)
					TaskTurnPedToFaceCoord(ped, tCoords.x, tCoords.y, tCoords.z, 3000)
					Wait(1000)
...
useMedbag = function()
	...
	loadModel('xm_prop_x17_bag_med_01a', 15000)
