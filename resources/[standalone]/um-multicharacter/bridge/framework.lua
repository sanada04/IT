...
if GetResourceState('qbx_core') ~= 'started' then
    ...
    if IsDuplicityVersion() then
        ...
        function Framework:RefreshCommand(src)
            return QBCore?.Commands.Refresh(src) or Debug('Refreshed Commands')
        end

        function Framework:Logout(src)
            return QBCore?.Player.Logout(src) or Debug('Logout Character')
        end
    end
else
    -- Functions Shared [um-multicharacter\framework\custom.lua]
    ...
    if IsDuplicityVersion() then
        ...
        function Framework:Logout(src)
            return QBX:Logout(src) or Debug('Logout Character ?')
        end
    end
end
