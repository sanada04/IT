...
function CheckAuth(source, silent)
    ...
    if jobName and Config and Config.PoliceJobs then
        for _, name in ipairs(Config.PoliceJobs) do
            if tostring(name) == tostring(jobName) then
                isPolice = true
                break
            end
        end
    ...
    if not isPolice and not isMedical then
        ...
        ps.debug('Access Granted for ID: ' .. source .. ', Name: ' .. ps.getPlayerName(source) .. ', job type: ' .. tostring(jobType) .. ', job name: ' .. tostring(jobName))
        return true
    end
end
