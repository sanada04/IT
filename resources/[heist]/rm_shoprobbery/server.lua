...
local function giveBlackMoneyQB(src, player, amount)
    ...
    if GetResourceState('ox_inventory') == 'started' then
        local ok, err = exports.ox_inventory:AddItem(src, 'black_money', amount)
        if ok then
            return true
        end
        print(('[rm_shoprobbery] failed to give black_money via ox_inventory: %s'):format(tostring(err)))
    end

    local info = { worth = amount }
    local added = player.Functions.AddItem('markedbills', 1, false, info)
    if added then
        local itemData = QBCore and QBCore.Shared and QBCore.Shared.Items and QBCore.Shared.Items['markedbills']
        if itemData then
                    ...
                    if item == 'safecrack' then
                        ...
                        if Config['enableLixeiroCharmosoMarkets'] then
                            ...
                            if query then
                                ...
                                if Config['ShopRobbery']['black_money'] then
                                    giveBlackMoneyQB(src, player, count)
