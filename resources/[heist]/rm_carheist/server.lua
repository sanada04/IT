...
Citizen.CreateThread(function()
    ...
    elseif Config['CarHeist']['framework']['name'] == 'QB' then
        while not QBCore do
            ...
            if not QBCore then
                TriggerEvent(Config['CarHeist']['framework']['eventName'], function(obj) QBCore = obj end)
            end
            Citizen.Wait(1)
        end
...
RegisterCommand('pdcarheist', function(source, args)
    ...
    if Config['CarHeist']['framework']['name'] == 'ESX' then
        ...
        if player then
            for k, v in pairs(Config['ArtifactHeist']['dispatchJobs']) do
                if player['job']['name'] == v then
                    for i = #cars['entity'], #cars['entity'] - 2, -1 do
                        DeleteEntity(cars['entity'][i])
                        cars['entity'][i] = nil
                    end
                    return TriggerClientEvent('carheist:client:sync', -1, 'resetHeist')
                end
            end
            
            TriggerClientEvent('artifactheist:client:showNotification', src, Strings['not_cop'])
        end
    elseif Config['CarHeist']['framework']['name'] == 'QB' then
        local player = QBCore.Functions.GetPlayer(src)
