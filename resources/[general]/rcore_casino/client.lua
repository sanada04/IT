...
ESX.Streaming = {
	RequestAnimDict = function(dict, cb)
...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('luckywheel:getcar', function(_car)
		car = _car
	end)
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.VehiclePos.x, Config.VehiclePos.y, Config.VehiclePos.z, true) < 40 then
			if DoesEntityExist(vehicle) == false then
				if car then
					...
					vehicle = CreateVehicle(GetHashKey(car), Config.VehiclePos.x, Config.VehiclePos.y, Config.VehiclePos.z, 0.0, false, false)
