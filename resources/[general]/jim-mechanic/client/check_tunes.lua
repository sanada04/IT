	...
	local CheckMenu = {
		...
		if GetNumVehicleMods(vehicle,11) ~= 0 then -- If engine can be changed
			if GetVehicleMod(vehicle, 11) == -1 then  -- If Stock
				...
				local item = getItemData("engine"..lvl)
				modEngine = "<img src=nui://"..Config.img..item.image.." width=13px onerror='this.onerror=null; this.remove();'> "..item.label..": [LVL "..lvl.." / "..GetNumVehicleMods(vehicle, 11).."]"
				modEngineHead = false
			end
		...
		if GetNumVehicleMods(vehicle, 12) ~= 0 then
			if GetVehicleMod(vehicle, 12) == -1 then
				...
				local item = getItemData("brakes"..lvl)
				modBrakes = "<img src=nui://"..Config.img..item.image.." width=13px onerror='this.onerror=null; this.remove();'> "..item.label..": [LVL "..lvl.." / "..GetNumVehicleMods(vehicle, 12).."]"
				modBrakesHead = false
			end
		...
		if GetNumVehicleMods(vehicle,15) ~= 0 then
			if GetVehicleMod(vehicle, 15) == -1 then
				...
				local item = getItemData("suspension"..lvl)
				modSuspension = "<img src=nui://"..Config.img..item.image.." width=13px onerror='this.onerror=null; this.remove();'> "..item.label..": [LVL "..lvl.." / "..GetNumVehicleMods(vehicle, 15).."]"
		...
		if GetNumVehicleMods(vehicle,13) ~= 0 then
			if GetVehicleMod(vehicle, 13) == -1 then
				...
				local item = getItemData("transmission"..lvl)
				modTransmission = "<img src=nui://"..Config.img..item.image.." width=13px onerror='this.onerror=null; this.remove();'> "..item.label.." [LVL "..lvl.." / "..GetNumVehicleMods(vehicle, 13).."]"
		...
		if GetNumVehicleMods(vehicle, 16) ~= 0 then
			if (GetVehicleMod(vehicle, 16)+1) == GetNumVehicleMods(vehicle, 16) then armicon = "car_armor"
				modArmor = "<img src=nui://"..Config.img..getItemData("car_armor").image.." width=13px onerror='this.onerror=null; this.remove();'> "..Loc[Config.Lan]["check"].reinforced
				modArmorHead = false
		...
		elseif IsToggleModOn(vehicle, 18) then turicon = "turbo"
			modTurbo = "<img src=nui://"..Config.img..getItemData("turbo").image.." width=13px onerror='this.onerror=null; this.remove();'> "..getItemData("turbo").label
			modTurboHead = false
		...
		elseif IsToggleModOn(vehicle, 22) then headicon = "headlights"
			modXenon = "<img src=nui://"..Config.img..getItemData("headlights").image.." width=13px onerror='this.onerror=null; this.remove();'> "..Loc[Config.Lan]["check"].xenoninst
			modXenonHead = false
		...
		if GetGameBuildNumber() >= 2372 then
			...
			elseif GetDriftTyresEnabled(vehicle) == 1 then drifticon = "drifttires"
				modDrift = "<img src=nui://"..Config.img..getItemData("drifttires").image.." width=13px onerror='this.onerror=null; this.remove();'> "..Loc[Config.Lan]["check"].tireinst
				modDriftHead = false
		...
		if GetVehicleTyresCanBurst(vehicle) == false and GetNumVehicleMods(vehicle,11) ~= 0 then bprooficon = "bprooftires"
			modBproof = "<img src=nui://"..Config.img..getItemData("bprooftires").image.." width=13px onerror='this.onerror=null; this.remove();'> "..Loc[Config.Lan]["check"].tireinst
			modBproofHead = false
		...
		if VehicleNitrous[trim(GetVehicleNumberPlateText(vehicle))] then
			local text = "<img src=nui://"..Config.img..getItemData("nos").image.." width=13px onerror='this.onerror=null; this.remove();'>"..nosBar(VehicleNitrous[trim(GetVehicleNumberPlateText(vehicle))].level).." "..math.floor(VehicleNitrous[trim(GetVehicleNumberPlateText(vehicle))].level).."%"
...
RegisterNetEvent('jim-mechanic:client:Menu:Remove', function(data)
	...
	if data.mod == "brakes" then icon = "brakes"..(GetVehicleMod(data.vehicle, 12)+1)
		header = "<img src=nui://"..Config.img..getItemData("brakes"..(GetVehicleMod(data.vehicle, 12)+1)).image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label49
		event = "jim-mechanic:client:giveBrakes"
	elseif data.mod == "engine" then icon = "engine"..(GetVehicleMod(data.vehicle, 11)+1)
		header = "<img src=nui://"..Config.img..getItemData("engine"..(GetVehicleMod(data.vehicle, 11)+1)).image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label50
		event = "jim-mechanic:client:giveEngine"
	elseif data.mod == "suspension" then icon = "suspension"..(GetVehicleMod(data.vehicle, 15)+1)
		header = "<img src=nui://"..Config.img..getItemData("suspension"..(GetVehicleMod(data.vehicle, 15)+1)).image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label51
	...
	elseif data.mod == "transmission" then icon = "transmission"..(GetVehicleMod(data.vehicle, 13)+1)
		header = "<img src=nui://"..Config.img..getItemData("transmission"..(GetVehicleMod(data.vehicle, 13)+1)).image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label52
		event = "jim-mechanic:client:giveTransmission"
	elseif data.mod == "armour" then icon = "car_armor"
		header = "<img src=nui://"..Config.img..getItemData("car_armor").image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label53
		event = "jim-mechanic:client:giveArmor"
	elseif data.mod == "turbo" then icon = "turbo"
		header = "<img src=nui://"..Config.img..getItemData("turbo").image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label54
		event = "jim-mechanic:client:giveTurbo"
	elseif data.mod == "xenon" then icon = "headlights"
		header = "<img src=nui://"..Config.img..getItemData("headlights").image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label55
		event = "jim-mechanic:client:giveXenon"
	elseif data.mod == "drift" then	icon = "drifttires"
		header = "<img src=nui://"..Config.img..getItemData("drifttires").image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label56
		event = "jim-mechanic:client:giveDrift"
	elseif data.mod == "bproof" then icon = "bprooftires"
		header = "<img src=nui://"..Config.img..getItemData("bprooftires").image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label56
		event = "jim-mechanic:client:giveBulletProof"
	elseif data.mod == "nos" then icon = "nos" setheader = Loc[Config.Lan]["check"].label57
		header = "<img src=nui://"..Config.img..getItemData("noscan").image.." width=50px onerror='this.onerror=null; this.remove();'>"..Loc[Config.Lan]["check"].label57
