...
-- ox_inventory direct-use wrappers
RegisterNetEvent('jim-mechanic:client:applyEngine1', function() TriggerEvent('jim-mechanic:client:applyEngine', 0) end)
RegisterNetEvent('jim-mechanic:client:applyEngine2', function() TriggerEvent('jim-mechanic:client:applyEngine', 1) end)
RegisterNetEvent('jim-mechanic:client:applyEngine3', function() TriggerEvent('jim-mechanic:client:applyEngine', 2) end)
RegisterNetEvent('jim-mechanic:client:applyEngine4', function() TriggerEvent('jim-mechanic:client:applyEngine', 3) end)
RegisterNetEvent('jim-mechanic:client:applyEngine5', function() TriggerEvent('jim-mechanic:client:applyEngine', 4) end)

RegisterNetEvent('jim-mechanic:client:applyTransmission1', function() TriggerEvent('jim-mechanic:client:applyTransmission', 0) end)
RegisterNetEvent('jim-mechanic:client:applyTransmission2', function() TriggerEvent('jim-mechanic:client:applyTransmission', 1) end)
RegisterNetEvent('jim-mechanic:client:applyTransmission3', function() TriggerEvent('jim-mechanic:client:applyTransmission', 2) end)
RegisterNetEvent('jim-mechanic:client:applyTransmission4', function() TriggerEvent('jim-mechanic:client:applyTransmission', 3) end)

RegisterNetEvent('jim-mechanic:client:applyBrakes1', function() TriggerEvent('jim-mechanic:client:applyBrakes', 0) end)
RegisterNetEvent('jim-mechanic:client:applyBrakes2', function() TriggerEvent('jim-mechanic:client:applyBrakes', 1) end)
RegisterNetEvent('jim-mechanic:client:applyBrakes3', function() TriggerEvent('jim-mechanic:client:applyBrakes', 2) end)

RegisterNetEvent('jim-mechanic:client:applySuspension1', function() TriggerEvent('jim-mechanic:client:applySuspension', 0) end)
RegisterNetEvent('jim-mechanic:client:applySuspension2', function() TriggerEvent('jim-mechanic:client:applySuspension', 1) end)
RegisterNetEvent('jim-mechanic:client:applySuspension3', function() TriggerEvent('jim-mechanic:client:applySuspension', 2) end)
RegisterNetEvent('jim-mechanic:client:applySuspension4', function() TriggerEvent('jim-mechanic:client:applySuspension', 3) end)
RegisterNetEvent('jim-mechanic:client:applySuspension5', function() TriggerEvent('jim-mechanic:client:applySuspension', 4) end)

local function getItemData(itemName)
	local item = QBCore.Shared and QBCore.Shared.Items and QBCore.Shared.Items[itemName]
	if not item then
		local ok, oxItem = pcall(function()
			return exports.ox_inventory:Items(itemName)
		end)
		if ok then item = oxItem end
	end
	if not item then item = {} end
	item.label = item.label or itemName
	item.image = item.image or (itemName .. ".png")
	return item
end
	...
	if DoesEntityExist(vehicle) then
		...
		if GetVehicleMod(vehicle, 16)+1 == GetNumVehicleMods(vehicle, 16) then triggerNotify(nil, Loc[Config.Lan]["common"].already, "error") else
			...
			{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
				...
				qblog("`"..getItemData("car_armor").label.." - car_armor` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:giveArmor', function()
	...
	{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
		...
		qblog("`"..getItemData("car_armor").label.." - car_armor` removed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:applyBrakes', function(level)
	...
	if DoesEntityExist(vehicle) then
		...
		if GetVehicleMod(vehicle, 12) == level then triggerNotify(nil, getItemData("brakes"..(level+1)).label.." "..Loc[Config.Lan]["common"].already, "error") else
			SetVehicleEngineOn(vehicle, false, false, true)
			...
			{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
				...
				qblog("`"..getItemData("brakes"..(level+1)).label.." - brakes"..(level+1) .."` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:giveBrakes', function()
	...
	{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function()
		...
		qblog("`"..getItemData("brakes"..(currentBrakes+1)).label.." - brakes"..(currentBrakes+1) .."` removed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:applySuspension', function(level)
	...
	if DoesEntityExist(vehicle) then
		...
		if currentSuspension == level then triggerNotify(nil, getItemData("suspension"..(level+1)).label.." "..Loc[Config.Lan]["common"].already, "error") else
			SetVehicleEngineOn(vehicle, false, false, true)
			...
			{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
				...
				qblog("`"..getItemData("suspension"..(level+1)).label.." - suspension"..(level+1).."` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:giveSuspension', function()
	...
	{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
		...
		qblog("`"..getItemData("suspension"..(currentSuspension+1)).label.." - suspension"..(currentSuspension+1).."` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:applyTransmission', function(level)
	...
	if DoesEntityExist(vehicle) then
		...
		if currentTrans == level then triggerNotify(nil, getItemData("transmission"..(level+1)).label.." "..Loc[Config.Lan]["common"].already, "error") else
			SetVehicleEngineOn(vehicle, false, false, true)
			SetVehicleDoorOpen(vehicle, 4, false, false)
			QBCore.Functions.Progressbar("drink_something", Loc[Config.Lan]["common"].installing.."LVL: "..level+1, math.random(7000,10000), false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = true, },
			{ animDict = "mini@repair",	anim = "fixing_a_ped", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
				...
				qblog("`"..getItemData("transmission"..(level+1)).label.." - transmission"..(level+1).."` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:giveTransmission', function()
	...
	{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
		...
		qblog("`"..getItemData("transmission"..(currentTrans+1)).label.." - transmission"..(currentTrans+1).."` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
	...
	if DoesEntityExist(vehicle) then
		...
		if IsToggleModOn(vehicle, 18) then triggerNotify(nil, Loc[Config.Lan]["common"].already, "error") else
			...
			{ animDict = "mini@repair",	anim = "fixing_a_ped", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
				...
				qblog("`"..getItemData("turbo").label.." - turbo` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:giveTurbo', function()
	...
	{ animDict = "mini@repair",	anim = "fixing_a_ped", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0) SetVehicleModKit(vehicle, 0)
		...
		qblog("`"..getItemData("turbo").label.." - turbo` removed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
	...
	if DoesEntityExist(vehicle) then
		...
		if GetVehicleMod(vehicle, 11) == level then triggerNotify(nil, "LVL: "..(level+1)..Loc[Config.Lan]["common"].already, "error") else
			...
			{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
				...
				qblog("`"..getItemData("engine"..(level+1)).label.." - engine"..(level+1).."` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('jim-mechanic:client:giveEngine', function()
	...
	{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
		...
		qblog("`"..getItemData("engine"..(currentEngine+1)).label.." - engine"..(currentEngine+1).."` removed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
