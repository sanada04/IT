...
RegisterNetEvent('jim-mechanic:client:Repair:Check', function(skip)
	...
	if skip == -2 then else
		...
		if not Config.FreeRepair then
			...
			if EngineRepair ~= 0 then costEngine = Loc[Config.Lan]["repair"].cost..EngineRepair.." "..getItemData(Config.RepairEngine).label end
			...
			if BodyRepair ~= 0 then costBody = Loc[Config.Lan]["repair"].cost..BodyRepair.." "..getItemData(Config.RepairBody).label end
			if useMechJob() then
				...
				if RadiatorRepair ~= 0 then costRadiator = Loc[Config.Lan]["repair"].cost..RadiatorRepair.." "..getItemData(Config.RepairRadiator).label end
				...
				if AxleRepair ~= 0 then costAxle = Loc[Config.Lan]["repair"].cost..AxleRepair.." "..getItemData(Config.RepairAxle).label end
				...
				if BrakesRepair ~= 0 then costBrakes = Loc[Config.Lan]["repair"].cost..BrakesRepair.." "..getItemData(Config.RepairBrakes).label or "" end
				...
				if ClutchRepair ~= 0 then costClutch = Loc[Config.Lan]["repair"].cost..ClutchRepair.." "..getItemData(Config.RepairClutch).label end
				...
				if FuelRepair ~= 0 then costFuel = Loc[Config.Lan]["repair"].cost..FuelRepair.." "..getItemData(Config.RepairFuel).label end
			end
		end
	end
	...
	local RepairMenu = {
		...
		if wheels >= 1 then
			local text = Loc[Config.Lan]["repair"].replacetire..Loc[Config.Lan]["repair"].cost..wheels.." "..getItemData("sparetire").label
