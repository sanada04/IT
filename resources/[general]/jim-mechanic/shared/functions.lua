...
local function applyItemFallback()
	if not QBCore or not QBCore.Shared then return end
	QBCore.Shared.Items = QBCore.Shared.Items or {}

	local itemsMeta = getmetatable(QBCore.Shared.Items) or {}
	if not itemsMeta.__jimItemFallbackInstalled then
		local previousIndex = itemsMeta.__index
		itemsMeta.__index = function(tbl, key)
			local previousItem = nil
			if type(previousIndex) == "function" then
				previousItem = previousIndex(tbl, key)
			elseif type(previousIndex) == "table" then
				previousItem = previousIndex[key]
			end
			if previousItem then return previousItem end
			if type(key) ~= "string" then return nil end

			local ok, oxItem = pcall(function()
				return exports.ox_inventory:Items(key)
			end)
			if not ok or not oxItem then return nil end

			local fallback = {
				name = key,
				label = oxItem.label or key,
				image = oxItem.image or (key .. ".png")
			}
			rawset(tbl, key, fallback)
			return fallback
		end
		itemsMeta.__jimItemFallbackInstalled = true
		setmetatable(QBCore.Shared.Items, itemsMeta)
	end
end
...
RegisterNetEvent('QBCore:Client:UpdateObject', refreshCoreObject)
RegisterNetEvent('QBCore:Client:SharedUpdate', function()
	applyItemFallback()
end)
applyItemFallback()
...
local mechStatusChecked, mechStatusSupported = false, false

local function hasMechStatusExport()
	if GetResourceState('qb-mechanicjob') ~= "started" then
		mechStatusChecked, mechStatusSupported = false, false
		return false
	end

	if mechStatusChecked then return mechStatusSupported end

	local ok = pcall(function()
		return exports['qb-mechanicjob']:GetVehicleStatus("__jim_probe__", "radiator")
	end)

	mechStatusChecked = true
	mechStatusSupported = ok

	if Config.Debug and not ok then
		print("^5Debug^7: ^1qb-mechanicjob export GetVehicleStatus not available, disabling extra damage integration^7")
	end

	return mechStatusSupported
end

function useMechJob()
	return hasMechStatusExport()
end

local jimItems = {
	"mechanic_tools", "toolbox", "ducttape", "mechboard", "turbo", "car_armor",
	"nos", "noscan", "noscolour", "engine1", "engine2", "engine3", "engine4", "engine5",
	"transmission1", "transmission2", "transmission3", "transmission4",
	"brakes1", "brakes2", "brakes3",
	"suspension1", "suspension2", "suspension3", "suspension4", "suspension5",
	"bprooftires", "drifttires", "underglow_controller", "headlights", "tint_supplies",
	"customplate", "hood", "roof", "spoiler", "bumper", "skirts", "exhaust", "seat",
	"rollcage", "rims", "livery", "paintcan", "tires", "horn", "internals", "externals",
	"newoil", "sparkplugs", "carbattery", "axleparts", "sparetire"
}
...
function HasItem(items, amount)
    ...
    local count = 0
    local kvIndex = 2
	local playerItems = (QBCore.Functions.GetPlayerData() and QBCore.Functions.GetPlayerData().items) or {}
    ...
    for _, itemData in pairs(playerItems) do
        if itemData and (itemData.name == items) then
			local itemCount = itemData.amount or itemData.count or 0
			if Config.Debug then print("^5Debug^7: ^3HasItem^7: ^2Item^7: '^3"..tostring(items).."^7' ^2Slot^7: ^3"..itemData.slot.." ^7x(^3"..tostring(itemCount).."^7)") end
			count += itemCount
		end
	end
