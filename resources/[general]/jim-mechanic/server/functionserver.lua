...
RegisterNetEvent("jim-mechanic:server:discordLog", function(info)
	if not Config.DiscordPreview then return end
	if type(info) ~= "table" or type(info.modlist) ~= "table" then return end
	if not isAllowedWebhook(info.htmllink) then
		if Config.Debug then print("^1jim-mechanic^7: blocked non-discord webhook url") end
		return
	end
