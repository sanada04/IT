    ...
    while true do
        ...
        if inRange and not shown then
        	...
        	if Config.okokTextUI then
        		exports['okokTextUI']:Open('[E] 銀行を開く', 'darkblue', 'left')
            else
        		TriggerEvent('QBCore:Notify', "Eキーで銀行を開く", "success")
        	end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if inRange and not shown then
        	shown = true
        	if Config.okokTextUI then
        		exports['okokTextUI']:Open('[E] ATMを開く', 'darkblue', 'left')
        	else
        		TriggerEvent('QBCore:Notify', "EキーでATMを開く", "success")
        	end
        elseif not inRange and shown then
        	shown = false
        	if Config.okokTextUI then
...
AddEventHandler("okokBanking:OpenATM", function()
	...
	QBCore.Functions.TriggerCallback("okokBanking:GetPIN", function(pin)
		if pin then
			...
			exports['okokNotify']:Alert("銀行", "PINコードを設定するには銀行窓口を利用してください", 5000, 'info')
		end
	end)
end)
	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				exports['okokNotify']:Alert("銀行", "金額が不正です", 5000, 'error')
			end
		else
			exports['okokNotify']:Alert("銀行", "入力が不正です", 5000, 'error')
		end
			...
			if tonumber(data.value) > 0 then
				QBCore.Functions.TriggerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer, name)
					...
					elseif isUsed == nil then
						exports['okokNotify']:Alert("銀行", "そのIBANは存在しません", 5000, 'error')
					end
				end, data.iban:upper())
			else
				exports['okokNotify']:Alert("銀行", "金額が不正です", 5000, 'error')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			QBCore.Functions.TriggerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					exports['okokNotify']:Alert("銀行", "このIBANはすでに使用されています", 5000, 'error')
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				exports['okokNotify']:Alert("銀行", "IBANは数字のみ使用できます", 5000, 'error')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				exports['okokNotify']:Alert("銀行", "PINコードは4桁で入力してください", 5000, 'info')
			end
		else
			exports['okokNotify']:Alert("銀行", "数字のみ入力できます", 5000, 'info')
		end
	end
end)
