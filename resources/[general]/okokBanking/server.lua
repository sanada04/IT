...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", amount.."€を入金しました", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "所持金が不足しています", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', _source, xPlayer.PlayerData.money.bank, xPlayer.PlayerData.money.cash)
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", amount.."€を出金しました", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "銀行残高が不足しています", 5000, 'error')
	end
end)
	...
	if xPlayer.PlayerData.citizenid ~= targetIdentifier then
		if amount <= playerMoney then
			...
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.PlayerData.citizenid == targetIdentifier then
				    	...
				    	TriggerClientEvent('okokNotify:Alert', xPlayers[i], "銀行", xPlayer.PlayerData.charinfo.firstname..' '..xPlayer.PlayerData.charinfo.lastname.." から "..amount.."€ を受け取りました", 5000, 'success')
				    end
				end
				TriggerEvent('okokBanking:AddTransferTransaction', amount, xTarget, _source)
				TriggerClientEvent('okokBanking:updateTransactions', _source, xPlayer.PlayerData.money.bank, xPlayer.PlayerData.money.cash)
				TriggerClientEvent('okokNotify:Alert', _source, "銀行", xTarget.PlayerData.charinfo.firstname..' '..xTarget.PlayerData.charinfo.lastname.." に "..amount.."€ を送金しました", 5000, 'success')
			elseif xTarget == nil then
				...
				TriggerClientEvent('okokBanking:updateTransactions', _source, xPlayer.PlayerData.money.bank, xPlayer.PlayerData.money.cash)
				TriggerClientEvent('okokNotify:Alert', _source, "銀行", targetName.." に "..amount.."€ を送金しました", 5000, 'success')
		...
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "自分自身には送金できません", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", societyName.." に "..amount.."€ を入金しました", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "所持金が不足しています", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			TriggerClientEvent('okokNotify:Alert', _source, "銀行", "他のユーザーが出金処理中です", 5000, 'error')
		else
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', _source, xPlayer.PlayerData.money.cash)
			TriggerClientEvent('okokNotify:Alert', _source, "銀行", societyName.." から "..amount.."€ を出金しました", 5000, 'success')
		end
	else
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "組織口座の残高が不足しています", 5000, 'error')
	end
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	local playerMoney = xPlayer.PlayerData.money.bank
		...
		if amount <= playerMoney then
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', _source, xPlayer.PlayerData.money.cash)
			TriggerClientEvent('okokNotify:Alert', _source, "銀行", societyName.." に "..amount.."€ を送金しました", 5000, 'success')
		else
			TriggerClientEvent('okokNotify:Alert', _source, "銀行", "銀行残高が不足しています", 5000, 'error')
		end
end)
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', _source, xPlayer.PlayerData.money.cash)
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", societyNameTarget.." に "..amount.."€ を送金しました", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "組織口座の残高が不足しています", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if amount <= xPlayer.PlayerData.money.bank then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "IBANを "..iban.." に変更しました", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "IBAN変更には "..amount.."€ が必要です", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if amount <= xPlayer.PlayerData.money.bank then
		...
		TriggerClientEvent('okokBanking:updateIbanPinChange', _source)
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "PINコードを "..pin.." に変更しました", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "銀行", "PIN変更には "..amount.."€ が必要です", 5000, 'error')
	end
end)
